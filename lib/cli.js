...
cli.responders.moreUserInfo = function(str) {
    ...
    var arr = str.split('--');
    var userId = typeof(arr[1]) == 'string' && arr[i].trim().length > 0 ? arr[i].trim() : false;
    if(userId) {
        // Lookup the user
        _data.read('users', userId, function(err, userData) {
            if(!err && userData) {
        ...
        if(!err && checkIds && checkIds.length > 0) {
            ...
            checkIds.forEach(function(checkId) {
                _data.read('checks', checkId, function(err, checkData) {
                    ...
                    var stateOrUnkonwn = typeof(checkData.state) == 'string' ? checkData.state : 'unknown';
                    // If the user has specified the state, or hasn't specified any state, include the current check accordingly
                    if(lowerString.indexOf('--'+state) > -1 || (lowerString.indexOf('--down') == -1 && lowerString.indexOf('--up') == -1)){
                        var line = 'ID: '+checkData.id+ ' ' +checkData.method.toUpperCase()+ ' ' +checkData.protocol+ '://' +checkData.url+ ' State: ' + stateOrUnkonwn;
                        console.log(line);
                        cli.verticalSpace();
                    }
                })
