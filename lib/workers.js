      ...
      checks.forEach(function (check) {
        ...
        _data.read("checks", check, function (error, originalCheckData) {
          if (!err && originalCheckData) {
            // Pass it to the check validator, and let that function continue or log orders as needed
...
workers.performCheck = function (originalCheckData) {
  ...
  // Parse the hostname and thepat out of the original check data
...
workers.processCheckOutcome = function (originalCheckData, checkOutcome) {
  ...
  var alertWarranted =
    originalCheckData.lastChecked && originalCheckData.state !== fs.stat
      ? true
      : false;
...
workers.alertUserToStatusChange = function (newCheckData) {
  ...
  helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
    if (!err) {
      debug(
        "Success: User was alerted to a status change in the check, via sms: ",
        msg
      );
    } else {
      debug(
