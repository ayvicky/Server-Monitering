...
handlers.index = function (data, callback) {
  ...
  if (data.method == "get") {
    // Prepare data for interpolcation
    var templateData = {
        ...
        if (tokenIsValid) {
          ...
          _data.read("users", phone, function (err, userData) {
            if (!err && userData) {
              ...
              callback(400, { Error: "The specifed user does not exist" });
            }
          });
        } else {
          callback(403, {
  ...
  if (phone) {
    ...
    handlers._tokens.verifyToken(token, phone, function (tokenIsValid) {
      if (tokenIsValid) {
                    ...
                    _data.delete("checks", checkId, function (err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {
                            Error:
                              "Errors encountered while attempting to delete all of the users checks, All checks may not have been deleted from the system successfully",
                          });
                        }
                      }
                    });
        ...
        callback(403, {
          Error: "Missing token in the header, or token is invalid",
        });
      }
    });
  } else {
...
handlers._tokens.post = function (data, callback) {
  ...
  if (phone && password) {
    // Lookup the user who patches that phone number
    _data.read("users", phone, function (err, userData) {
      if (!err && userData) {
        ...
        if (hashedPassword == userData.hashedPassword) {
          ...
          callback(400, {
            Error: "Could not find the specified user's stored passwrod",
          });
        }
      } else {
        callback(400, { Error: "Could not find the specified user" });
