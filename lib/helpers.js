...
helpers.sendTwilioSms = function (phone, msg, callback) {
  ...
  if (phone && msg) {
    ...
    var req = https.request(requestDetails, function (res) {
      ...
      // Callback successfully, if request would not throw
    ...
    // Bind to the error event so if doesn't get thrown
    ...
    callback("Given paramters were missing or invalids");
  }
};
  ...
  if (templateName) {
    ...
    fs.readFile(templateDir + templateName + ".html", "utf8", function (
      ...
      if (!err && str && str.length > 0) {
        // On interpolation on the string
...
helpers.interpolate = function(str, data) {
  ...
  // for each key in the data object, insert it's value into the string at the corresponding placement
  for(var key in data) {
